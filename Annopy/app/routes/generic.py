...
def sign_up():
    ...
    # Si la requête HTTP est POST et qu'un formulaire html a donc été envoyé, on crée un compte utilisateur-ice.
...
def sign_in():
    ...
    # Si l'utilisateur-ice est connectée, on envoie un message au template avec flash()
...
def profile():
    ...
    L'utilisateur-ice doit être connecté-e pour accéder à cette page. (décorateur login_required)
